/mob/monkey/New()
	spawn(1)
		if (!(src.primarynew))
			...
			src.primarynew.uni_identity = "00600200A00E0110148FC01300B009"
			...
			var/genderrand2
			if (src.gender == "male")
				genderrand = rand(1,2049)
				genderrand2 = add_zero2(num2hex((genderrand),1), 3)
			else
				genderrand = rand(2051,4094)
				genderrand2 = add_zero2(num2hex((genderrand),1), 3)
			src.primarynew.uni_identity += genderrand2
			src.primarynew.uni_identity += "12C"
			src.primarynew.uni_identity += "4E2"
...
/mob/monkey/Topic(href, href_list)
	...
	if (href_list["mach_close"])
		...
		src.machine = null
		src << browse(null, t1)
	if ((href_list["item"] && !( usr.stat ) && !( usr.restrained() ) && (get_dist(src, usr) <= 1 || usr.telekinesis == 1)))
