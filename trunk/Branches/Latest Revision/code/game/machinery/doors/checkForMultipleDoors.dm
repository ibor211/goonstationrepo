/obj/machinery/door/proc/checkForMultipleDoors()
	...
	for(var/obj/machinery/door/D in src.loc)
		if(!istype(D, /obj/machinery/door/window) && D.density)
			return 0
	if(istype(src.loc, /turf/station/wall/false_wall) && src.loc.density)
		return 0
	if(istype(src.loc, /turf/station/wall/false_rwall) && src.loc.density)
		return 0
	return 1

/turf/station/wall/proc/checkForMultipleDoors()
	if(!src.loc)
		return 0
