...
/obj/machinery/scan_consolenew/Topic(href, href_list)
	...
	if ((usr.contents.Find(src) || (get_dist(src, usr) <= 1 || usr.telekinesis == 1) && istype(src.loc, /turf)) || (istype(usr, /mob/ai)))
		...
		if (href_list["strucmenu"])
			...
			src.delete = 0
			//"[getleftblocks(src.connected.occupant.primarynew.struc_enzyme,strucblock,3)][src.subblock == 1 ? "<B>"+getblock(getblock(src.connected.occupant.primarynew.struc_enzyme,src.strucblock,3),1,1)+"</B>" : getblock(getblock(src.connected.occupant.primarynew.struc_enzyme,src.strucblock,3),1,1)][src.subblock == 2 ? "<B>"+getblock(getblock(src.connected.occupant.primarynew.struc_enzyme,src.strucblock,3),2,1)+"</B>" : getblock(getblock(src.connected.occupant.primarynew.struc_enzyme,src.strucblock,3),2,1)][src.subblock == 3 ? "<B>"+getblock(getblock(src.connected.occupant.primarynew.struc_enzyme,src.strucblock,3),3,1)+"</B>" : getblock(getblock(src.connected.occupant.primarynew.struc_enzyme,src.strucblock,3),3,1)]"
			//world << "Block: [getblock(src.connected.occupant.primarynew.struc_enzyme,src.strucblock,3)]"
			//world << "Sub-Block: [getblock(getblock(src.connected.occupant.primarynew.struc_enzyme,src.strucblock,3),src.subblock,1)]"
		...
		if (href_list["b1addui"])
			...
			if (!istype(src.connected.occupant,/mob/human))
				src.buffer1owner = src.connected.occupant.name
			else
				src.buffer1owner = src.connected.occupant.rname
		...
		if (href_list["b2adduiue"])
			...
			if (!istype(src.connected.occupant,/mob/human))
				...
				src.buffer2owner = src.connected.occupant.rname
		...
		if (href_list["b3adduiue"])
			...
			if (!istype(src.connected.occupant,/mob/human))
				...
				src.buffer3owner = src.connected.occupant.rname
		...
		if (href_list["b1transfer"])
			...
			if (src.buffer1type == "ui")
				if (src.buffer1iue)
					src.connected.occupant.rname = src.buffer1owner
		...
		if (href_list["b2transfer"])
			...
			if (src.buffer2type == "ui")
				if (src.buffer2iue)
					src.connected.occupant.rname = src.buffer2owner
		...
		if (href_list["b3transfer"])
			...
			if (src.buffer3type == "ui")
				if (src.buffer3iue)
					src.connected.occupant.rname = src.buffer3owner
