...
/obj/mine/proc/explode(obj)
	...
	pt.gas.temperature = 425+T0C
	pt.ignite()
	spawn(0)
		del(src)

/obj/mine/proc/sense()
	for(var/atom/A in view(range, src.loc))
		if(A!=src && istype(A, /mob/human) || istype(A, /mob/monkey))
			spawn(0)
				for(var/mob/O in viewers(world.view, src.loc))
					O << text("<font color='red'>[A] triggered the \icon[] [src]</font>", src)
			//explode or whatever
				call(src,triggerproc)(A)
			break
	spawn(senserate)
		sense()
...
/obj/mine/New()
	spawn(0)
		for(var/mob/O in viewers(world.view, src.loc))
			O << text("<font color='red'>\icon[] [src] now arming ([readytime / 10] seconds).</font>", src)
		spawn(readytime)
...
/obj/item/weapon/assembly/anal_ignite/attackby(obj/item/weapon/W as obj, mob/user as mob)
	if ((istype(W, /obj/item/weapon/wrench) && !( src.status )))
		var/turf/T = src.loc
		if (ismob(T))
			T = T.loc
	...
	if (( istype(W, /obj/item/weapon/screwdriver) ))
		...
		src.part2.status = src.status
		src.add_fingerprint(user)
	if(( istype(W, /obj/item/weapon/clothing/suit/armor) ) && src.status)
		var/obj/item/weapon/assembly/a_i_a/R = new /obj/item/weapon/assembly/a_i_a( user )
		W.loc = R
		R.part1 = W
		R.part2 = W
...
/*	else if ((istype(W, /obj/item/weapon/timer) && !( src.status )))
		...
		src.add_fingerprint(user)
*/
/obj/item/weapon/assembly/a_i_a/attackby(obj/item/weapon/W as obj, mob/user as mob)
	if ((istype(W, /obj/item/weapon/wrench) && !( src.status )))
		var/turf/T = src.loc
		if (ismob(T))
			T = T.loc
...
/obj/item/weapon/assembly/a_i_a/Del()
	//src.part1 = null
	del(src.part1)
	//src.part2 = null
	del(src.part2)
...
/obj/item/weapon/clothing/suit/a_i_a_ptank/attackby(obj/item/weapon/W as obj, mob/user as mob)
	if (istype(W, /obj/item/weapon/analyzer))
		src.part4.attackby(W, user)

	if ((istype(W, /obj/item/weapon/wrench) && !( src.status )))
		var/obj/item/weapon/assembly/a_i_a/R = new /obj/item/weapon/assembly/a_i_a(  )
		R.part1 = src.part1
		R.part2 = src.part2
		R.part3 = src.part3
		R.loc = src.loc
