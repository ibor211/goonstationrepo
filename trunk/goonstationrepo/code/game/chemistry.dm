...
/obj/item/weapon/chem/beaker/attack(mob/M as mob, mob/user as mob)
	if (!istype(M, /mob))
		return
	...
	if (user)
		for(var/mob/O in viewers(M, null))
			...
			//Foreach goto(192)
		user.show_message(text("\red You throw the beaker containing [] units at []'s face! The beaker is now broken.", src.chem.volume(), M))
